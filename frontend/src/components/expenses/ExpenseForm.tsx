...
export function ExpenseForm({ expense, onSubmit, onCancel }: ExpenseFormProps) {
  const [amount, setAmount] = useState(expense ? String(toDisplayAmount(expense.amount)) : '')
  const [date, setDate] = useState(
    () => (expense ? toInputDate(expense.date) || todayString() : todayString()),
  )
  ...
  useEffect(() => {
    getCategories('expense').then((cats) => {
      setCategories(cats)
      if (!expense && cats.length > 0 && categoryId === 0) {
