...
| **M0-02** | Environment Configuration | ✅ Done | `backend/internal/config/config.go` — `Config` struct + `Load()` reads all 11 env vars with defaults. `JWT_SECRET=""` logs a warning and falls back to dev default; `JWT_SECRET="change-me"` logs a separate warning. `.env.example` (root) has all 11 vars. `frontend/.env.example` has `VITE_API_BASE_URL`. `cmd/server/main.go` uses `config.Load()`. |
...
| **M0-06** | Health Check Endpoint Enhancement | ✅ Done | `backend/internal/handlers/health.go` — `GET /api/health` pings DB (`db.PingContext`) and MinIO (`storage.HealthCheck`). Returns `{"status":"ok","database":"ok","storage":"ok"}` (200), `{"status":"degraded",...}` (200) when MinIO fails, or 503 when DB fails. Registered via `NewHealthHandler`. |
...
3. **M0-03 — `decodeJSON` ResponseWriter argument**: `http.MaxBytesReader` is called with `nil` as the first argument instead of `w`. This prevents the library from automatically sending a 413 status; the error is still returned and handled, so behaviour is correct but the auto-413 path is bypassed.
